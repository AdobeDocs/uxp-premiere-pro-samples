...
/**
 * Get a caption track by index
 */
export async function getCaptionTrack(sequence: Sequence, trackIndex: number) {
  if (sequence) {
    const captionTrackCount = await sequence.getCaptionTrackCount();
    if (trackIndex >= captionTrackCount) {
      log(`Caption track index should be less than ${captionTrackCount}`);
      return;
    }
    return await sequence.getCaptionTrack(trackIndex);
  } else {
    log("No sequence found.");
  }
}
...
/**
 * Get detailed information about all caption tracks
 */
export async function getCaptionTrackInfo(sequence: Sequence) {
  if (sequence) {
    const captionTrackCount = await sequence.getCaptionTrackCount();
    if (captionTrackCount === 0) {
      log("No caption tracks found in the sequence", "orange");
      return [];
    }

    const trackInfos = [];
    for (let i = 0; i < captionTrackCount; i++) {
      const captionTrack = await sequence.getCaptionTrack(i);
      ...
      const trackInfo = {
        ...
        id: captionTrack.id,
        trackIndex: await captionTrack.getIndex(),
        isMuted: await captionTrack.isMuted(),
        mediaType: await captionTrack.getMediaType(),
      };
...
/**
 * Toggle the mute state of a caption track
 */
export async function toggleCaptionTrackMute(sequence: Sequence, trackIndex: number = 0) {
  if (sequence) {
    const captionTrackCount = await sequence.getCaptionTrackCount();
    if (captionTrackCount === 0) {
      log("No caption tracks found to mute/unmute", "orange");
      return false;
    }

    if (trackIndex >= captionTrackCount) {
      log(`Caption track index should be less than ${captionTrackCount}`, "red");
      return false;
    }

    const captionTrack = await sequence.getCaptionTrack(trackIndex);
    const currentMuteState = await captionTrack.isMuted();
    const newMuteState = !currentMuteState;
    
    const success = await captionTrack.setMute(newMuteState);
    if (success) {
      log(`Caption track "${captionTrack.name}" ${newMuteState ? 'muted' : 'unmuted'} successfully`, "green");
    } else {
      log(`Failed to ${newMuteState ? 'mute' : 'unmute'} caption track`, "red");
    }
...
/**
 * Get track items from all caption tracks
 */
export async function getCaptionTrackItems(sequence: Sequence) {
  if (sequence) {
    const captionTrackCount = await sequence.getCaptionTrackCount();
    if (captionTrackCount === 0) {
      log("No caption tracks found", "orange");
      return [];
    }
    ...
    for (let i = 0; i < captionTrackCount; i++) {
      ...
      // Get all track items (including empty ones)
      const allItems = await captionTrack.getTrackItems(
        ppro.Constants.TrackItemType.CLIP, 
        true // include empty track items
      );
      
      // Get only clip track items (excluding empty ones)
      const clipItems = await captionTrack.getTrackItems(
        ppro.Constants.TrackItemType.CLIP, 
        false // exclude empty track items
      );
      
      const trackData = {
        trackIndex: i,
        trackName: captionTrack.name,
        allItemsCount: allItems.length,
        clipItemsCount: clipItems.length,
        allItems: allItems,
        clipItems: clipItems
      };
      
      allTrackItems.push(trackData);
    }
