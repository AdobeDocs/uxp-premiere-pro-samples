...
export declare type EncoderManager = {
  exportSequence(sequence: object, exportType: Constants.ExportType, outputFile?: string, presetFile?: string, exportFull?: boolean): boolean	//Export a sequence. If no output file and preset is specified, the sequence will be exported with the applied export settings or standard export rules will be applied.
  isAMEInstalled: boolean	//Check if AME is installed.
}

export declare type ExporterStatic = {
  exportSequenceFrame(sequence: object, time: TickTime, filename: string, filepath: string, width: number, height: number): boolean	//Exports from a sequence. Supported formats are bmp, dpx, gif, jpg, exr, png, tga and tif
}
...
export declare type MetadataStatic = {
  ...
  addPropertyToProjectMetadataSchema(name: string, label: string, type: number): boolean	//Add name and label property to project metadata schema
...
export declare type Project = {
  ...
  createSequenceFromMedia(name: string, clipProjectItems: [], targetBin?: ProjectItem): Sequence	//Create a new sequence with a given name and medias
  ...
  importAEComps(aepPath: string, compNames: [], TargetBin?: ProjectItem): boolean
  importAllAEComps(aepPath: string, TargetBin?: ProjectItem): boolean
  importFiles(filePaths: [], suppressUI?: boolean, targetBin?: ProjectItem, asNumberedStills?: boolean): boolean	//Import files in root/target bin of the project
