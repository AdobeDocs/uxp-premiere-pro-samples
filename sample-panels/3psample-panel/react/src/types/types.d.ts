...
export declare type AudioComponentChain = {
  ...
  insertComponent(component: object, componentInsertionIndex: number): boolean; //Execute "Insert Component" undoable action and return true if successful.
  appendComponent(component: object): boolean; //Execute "Append Component" undoable action and return true if successful.
  removeComponent(component: object): boolean; //Execute "Remove Component" undoable action and return true if successful.
};
...
export declare type ClipProjectItem = {
  ...
  createSetInOutPointsAction(inPoint: TickTime, outPoint: TickTime): Action; //Returns an action which sets the in point of the Project item
  createClearInOutPointsAction(): Action; //Returns an action which clear the in or out point of the Project item
...
export declare type FolderItem = {
  ...
  createBin(name: string, makeUnique: boolean): ProjectItem; //Creates a new bin with given name and returns the Folder object. If second param (makeUnique) is true, we make sure the newly created bin has a unique name.
  ...
  removeItem(item: ProjectItem): boolean; //Removes the given item from provided folder and returns a promise
  ...
  moveItem(item: ProjectItem, newParent: FolderItem): boolean; //Moves the given project item to the provided folder item and returns a promise
...
export declare type ProjectStatic = {
  ...
  getActiveProject(): Project; //Currently active project.
...
export declare type Project = {
  ...
  openSequence(sequence: Sequence): boolean; //Open a sequence and return true if successful. Additionally, attempt to open the active sequence also return true.
...
export declare type Sequence = {
  ...
  id: Guid; //The unique identifier of the sequence.
};
