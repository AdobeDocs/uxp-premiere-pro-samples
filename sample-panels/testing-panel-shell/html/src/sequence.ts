...
export async function addHandlesToTrackItem(
  ...
  if(trackItem_toChange){
    ...
    try{
      ...
      let func_startTime = await trackItem_toChange.getStartTime(); 
      let func_endTime = await trackItem_toChange.getEndTime();
      let func_inPoint = await trackItem_toChange.getInPoint();
      let func_outPoint = await trackItem_toChange.getOutPoint();
